...
namespace Delta.PECS.WebService.DataServer
{
    public partial class MainFrm : Form
    {
        ...
        /// <param name="dt">dt</param>
        /// <param name="msg">msg</param>
        private void WriteGridLog(DateTime dt, string msg) {
            try {
                var line = String.Format("{0}  {1}{2}", dt.ToString("MM/dd HH:mm:ss"), msg, Environment.NewLine);
                if (this.InvokeRequired) {
                    this.Invoke(new MethodInvoker(delegate {
                        if (LogTextBox.Lines.Length >= 10000) { LogTextBox.Clear(); }
