using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Configuration;
using System.Reflection;

namespace Delta.PECS.WebService.DALFactory
{
    public static class ServiceLocator
    {
        private static readonly string dalPath = ConfigurationManager.AppSettings["DataServerDAL"];

        /// <summary>
        /// Locate DAL Object
        /// </summary>
        /// <param name="className">className</param>
        /// <returns>class Entity</returns>
        public static object LocateDALObject(string className)
        {
            string fullPath = String.Format("Delta.PECS.WebService.{0}.{1}", dalPath, className);
            return Assembly.Load(dalPath).CreateInstance(fullPath);
        }
    }
}
